...
Wires = []
for line in L:
    first, Box = line.split(": ")
    for wire in Box.split(" "):
        D[first].append(wire)
        D[wire].append(first)
        Wires.append((first, wire))
    startingNode = first

def isSameEdge(t1, t2):
    return (t1[0], t2[0]) == (t2[0], t1[0])

def findShortestPath(Connected, end, Omitted):
    Paths = []
    ...
    while len(Paths) > 0:
        path = Paths.pop(0)
        prevNode = path[-1][1]
        for neighbor in D[prevNode]:
            if neighbor in Found:
                continue
            if (prevNode, neighbor) in Omitted or (neighbor, prevNode) in Omitted:
                continue

            newPath = path + [(prevNode, neighbor)]
            if neighbor == end:
                return newPath
            else:
...
while len(Current) > 0:
    ...
    for neighbor in D[current]:
        if neighbor not in Joint and neighbor not in BadNodes:
            Omitted = set([(current, neighbor)]) | BadWires
            for _ in range(iterationTurns):
                path = findShortestPath(Joint, neighbor, Omitted)
                if not path:
                    break
                Omitted = Omitted | set(path)

            path = findShortestPath(Joint, neighbor, Omitted)
            if not path:
                BadNodes.add(neighbor)
                BadWires.add((current, neighbor))
                iterationTurns -= 1
            else:
                Joint.add(neighbor)
...
print(len(Joint) * (len(D) - len(Joint)))
...
while len(Current) > 0:
    ...
    for current in Current:
        for neighbor in D[current]:
            if neighbor in Found:
                if Found[neighbor] == level - 2:
                    continue
                else:
                    print(level + Found[neighbor])
                    print(neighbor)
                    sys.exit()
            else:
                Found[neighbor] = level
...
keys = list(D.keys())
wireCount = len(D)
for x in range(wireCount):
    if x % 1 == 0:
        print(x, time.time() - t1)
    for y in range(x + 1, wireCount):
        for z in range(y + 1, wireCount):
            Found = set([keys[x], keys[y], keys[z]])
            Neighbors = set(D[keys[x]])
            ...
            while len(Current) > 0:
                current = Current.pop(0)
                Found.add(current)
                for wire in D[current]:
                    if wire not in Found:
                        Found.add(wire)
                        Current.append(wire)
                if len(Neighbors - Found) == 0:
                    valid = False
                    break 
            if valid and len(Found) != wireCount:
                print((len(Found) - 3) * (wireCount - len(Found)))
                sys.exit()
'''
