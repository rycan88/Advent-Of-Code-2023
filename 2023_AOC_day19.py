...
Rules = {}
for rule in R:
    open_brac = rule.index("{")
    name = rule[:open_brac]
    Box = rule[open_brac + 1:-1].split(",")

    Conditions = []
    for subrule in Box[:-1]:
        colon = subrule.index(":")
        part = Let[subrule[0]]
        op = subrule[1]
        value = int(subrule[2:colon])
        dest = subrule[colon + 1:]
        Conditions.append((part, op, value, dest))
    Conditions.append(Box[-1])
...
for part in Parts:
    ...
    while current not in "AR":
        ...
        for condition in Rules[current][:-1]:
            current_num = part[condition[0]]
...
def product(Possible):
    total = 1
    for start, end in Possible:
        total *= end - start - 1
    return total

def recursion(current, Possible):
    if current == "R":
        return 0
    elif current =="A":
        return product(Possible)
    ...
    for condition in Rules[current][:-1]:
        ...
        target_num = condition[2]
        if condition[1] == ">":
            if Possible[i][1] - target_num <= 1:
                Possible[i][1] = target_num + 1
                continue
            New = deepcopy(Possible)
            New[i][0] = target_num
            Possible[i][1] = target_num + 1
        else:
            if target_num - Possible[i][0] <= 1:
                Possible[i][0] = target_num - 1
                continue
            New = deepcopy(Possible)
            New[i][1] = target_num
            Possible[i][0] = target_num - 1
        count += recursion(condition[3], New)
    final_dest = Rules[current][-1]
    count += recursion(final_dest, Possible)
...
Possible = [] 
for x in range(4):
    Possible.append([0, 4001])
print(recursion(current, deepcopy(Possible)))
