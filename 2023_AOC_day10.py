inp = '''J7F--7-|7FJ.FF|.FJ77.FJFL7FFF-J7F7-J-77F|-|7LL7.|FJ7F|JF|.FLF7-FF--7.-J.7-FF7F-77.LJFJ-FF.F|-F----77F77.JF|F|7F.|7-FJ-F777..F-7|77F7FFFF7FJ7
...
def getNextPair(current, dir):
    new = add(current, dir)
    if new == start:
        return start, startDir
    for new_dir in Pipes[D[new]]:
        if opposite(dir) != new_dir:
            return new, new_dir
...
right = (1, 0)
left = (-1, 0)
Pipes = {"|": (up, down), "-": (left, right), "L": (up, right), "J": (up, left), "7": (left, down), "F": (right, down), ".": ()}

for adjacent in (up, down, left, right):
    new  = add(start, adjacent)
    if new in D:
        found = False
        for dir in Pipes[D[new]]:
            if opposite(adjacent) == dir:
                found = True
                startDir = adjacent
        if found:
            break
...
current = start
currentDir = startDir
current, currentDir = getNextPair(current, currentDir)
count = 1
while current != start:
    current, currentDir = getNextPair(current, currentDir)
    count += 1
...
def getGroup(currentList, Set):
    for tup in currentList:
        for adjacent in Around:
            new = add(tup, adjacent)
            if new in D and new not in Set and new not in Loop:
                Set.add(new)
                currentList.append(new)
    return Set

def addRightLeft(current, currentDir):
    rightDir = Around[(Around.index(currentDir) + 1) % 4]
    rightTup = add(current, rightDir)
    if rightTup in D:
        Right.add(rightTup)
    
    leftDir = Around[(Around.index(currentDir) - 1) % 4]
    leftTup = add(current, leftDir)
    if leftTup in D:
        Left.add(leftTup)
...
while current != start:
    Loop.add(current)
    addRightLeft(current, currentDir)
    
    oldDir = currentDir
    current, currentDir = getNextPair(current, currentDir)
    if D[current] not in ("|", "-"):
        addRightLeft(current, oldDir)

Right -= Loop
Left -= Loop

RightGroup = getGroup(list(Right), Right.copy())
LeftGroup = getGroup(list(Left), Left.copy())

print(min(len(LeftGroup), len(RightGroup)))
